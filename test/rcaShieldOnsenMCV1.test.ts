...
describe("RcaShieldOnsen:LIDOWETH", function () {
  ...
  beforeEach(async function () {
    ...
    // impersonate lidoWethWhale

    await hre.network.provider.send("hardhat_impersonateAccount", [MAINNET_ADDRESSES.accounts.lidoWethWhale]);
    await hre.network.provider.send("hardhat_impersonateAccount", [MAINNET_ADDRESSES.accounts.sushiWhale]);
    signers.user = await ethers.getSigner(MAINNET_ADDRESSES.accounts.lidoWethWhale);
    ...
    contracts.uToken = <MockERC20>(
      await ethers.getContractAt("MockERC20", MAINNET_ADDRESSES.contracts.onsen.lidoWethPair)
    );
    // sushi token
    sushiToken = <MockERC20>await ethers.getContractAt("MockERC20", MAINNET_ADDRESSES.contracts.onsen.sushiToken);

    // // sushi masterchefV1
    // masterChefV1 = <IMasterChef>(
  ...
  describe("Initialize", function () {
    it("should initialize the shield with valid state", async function () {
      expect(await contracts.rcaShieldOnsen.pid()).to.be.equal(LIDOWETH_PID);
      expect(await contracts.rcaShieldOnsen.masterChef()).to.be.equal(masterChef.address);
    });
  });
  describe("mintTo()", function () {
    it("should deposit lp tokens to masterchef v1 after mint", async function () {
      ...
      let shieldDepositInfo = await masterChef.userInfo(LIDOWETH_PID, shieldAddress);
      ...
      // details of the deposited bitWeth lp and it's rewards
      // returns deposited amount and rewardDebt
      shieldDepositInfo = await masterChef.userInfo(LIDOWETH_PID, shieldAddress);

      // sum to two rca's buys
      const totalUDepositedToShield = uAmount.add(ether("10"));
      // deposited lp token should be equal to the amount of uAmount used to buy rca
  ...
  describe("purchase()", function () {
    ...
    it("should allow user to buy harvested sushi and deposit uTokens to sushi masterchef", async function () {
      ...
      */
      ...
      const sushiPriceProof = merkleTrees.priceTree1.getProof(sushiToken.address, sushiPrice);
      // buy sushi reward token for referrer signer
      const sushiAmtToBuy = ether("10");
      const userAddress = signers.referrer.address;
      const referrerSushiBalBefore = await sushiToken.balanceOf(userAddress);
      const shieldDepositInfoBefore = await masterChef.userInfo(LIDOWETH_PID, shieldAddress);
      ...
      const shieldDepositInfoAfter = await masterChef.userInfo(LIDOWETH_PID, shieldAddress);
  ...
  describe("redeemRequest()", function () {
    it("should withdraw lp tokens from sushi masterchef", async function () {
      ...
      const expectedUTokenToWithdraw = ether("1");
      const shieldAddress = contracts.rcaShieldOnsen.address;
