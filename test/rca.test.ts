...
describe("RCAs and Controller", function () {
  ...
  beforeEach(async function () {
    ...
    await controller
      .connect(owner)
      .initializeShield(shield.address, [1, 2], [10000, 10000]);
  ...
  describe("Initialize", function () {
    ...
    it("should initialize controller correctly", async function () {
      ...
      expect(await controller.priceOracle()).to.be.equal(
        await priceOracle.getAddress(),
      );
      expect(await controller.capOracle()).to.be.equal(
        await capOracle.getAddress(),
      );
      ...
      const protocolPercents0 = await controller.shieldProtocolPercents(
        shield.address,
        0,
      );
      const protocolPercents1 = await controller.shieldProtocolPercents(
        shield.address,
        1,
      );
  ...
  describe("Mint", function () {
    ...
    it.only("should be able to mint an RCA token", async function () {
      ...
      const nonce = await controller.nonces(user.getAddress());
      const timestamp = await getTimestamp();
      const expiry = timestamp.add(300);
      const hash = await controller.getMessageHash(
        user.getAddress(),
        shield.address,
        amount,
        nonce,
      ...
      const signature = await capOracle.signMessage(
    ...
    it("should block mints over capacity", async function () {
      ...
      ).to.be.revertedWith("Not enough capacity available.");
    });
    ...
    it("should mint correctly with wonky (technical term) updates", async function () {
      ...
      await controller
        .connect(owner)
        .setLiqTotal(liqTree.getHexRoot(), resTree.getHexRoot());
  ...
  describe("Redeem", function () {
    ...
    it("should be able to initiate and finalize redeem of RCA token", async function () {
      await shield
        .connect(user)
        .redeemRequest(ether("100"), 0, liqProof2, 0, resProof);
  ...
  describe("Purchase", function () {
    ...
    it("should purchase an RCA token from liquidation", async function () {
      ...
      expect(await shield.balanceOf(user.getAddress())).to.be.equal(
        "1055555555555555555555",
      );
    ...
    it("should purchase underlying tokens from liquidation", async function () {
      ...
      expect(await uToken.balanceOf(user.getAddress())).to.be.equal(
        ether("999050"),
      );
  ...
  describe("Views", function () {
    beforeEach(async function () {
      ...
      await controller
        .connect(owner)
        .setLiqTotal(liqTree2.getHexRoot(), resTree.getHexRoot());
      await shield
        .connect(user)
        .mintTo(
          user.getAddress(),
  ...
  describe("Privileged", function () {
    it("should block from privileged functions", async function () {
      await expect(
        controller.connect(user).setWithdrawalDelay(100000),
      ).to.be.revertedWith("msg.sender is not owner");
      await expect(
        controller.connect(user).setDiscount(1000),
      ).to.be.revertedWith("msg.sender is not owner");
      await expect(controller.connect(user).setApr(1000)).to.be.revertedWith(
        "msg.sender is not owner",
      );
      await expect(
        controller.connect(user).setTreasury(user.getAddress()),
      ).to.be.revertedWith("msg.sender is not owner");
      await expect(
        controller.connect(owner).setPercentReserved(resTree.getHexRoot()),
      ).to.be.revertedWith("msg.sender is not Guardian");

      await expect(
        shield.connect(user).setWithdrawalDelay(100000),
      ).to.be.revertedWith("Function must only be called by controller.");
      await expect(shield.connect(user).setDiscount(1000)).to.be.revertedWith(
        "Function must only be called by controller.",
      );
      await expect(shield.connect(user).setApr(1000)).to.be.revertedWith(
        "Function must only be called by controller.",
      );
      await expect(
        shield.connect(user).setTreasury(user.getAddress()),
      ).to.be.revertedWith("Function must only be called by controller.");
      await expect(
        shield.connect(owner).setPercentReserved(1000),
      ).to.be.revertedWith("Function must only be called by controller.");

      await expect(
        controller.connect(owner).setPrices(priceTree.getHexRoot()),
      ).to.be.revertedWith("msg.sender is not price oracle");

      await expect(
        shield.connect(user).setController(owner.getAddress()),
      ).to.be.revertedWith("msg.sender is not owner");
      await expect(
        shield.connect(user).proofOfLoss(owner.getAddress()),
      ).to.be.revertedWith("msg.sender is not owner");
    });
  });
});
