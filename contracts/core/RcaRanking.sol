...
contract RcaRanking {
    ...
    event Stake(uint256 indexed protocol, address indexed user, uint256 amount, uint256 newTotal);
    /// @notice Notification of an unstake..
    event Unstake(uint256 indexed protocol, address indexed user, uint256 amount, uint256 newTotal);
     ...
     */
    function stake(uint256 _protocol, uint256 _amount) external {
        token.safeTransferFrom(msg.sender, address(this), _amount);
        ranks[_protocol] += _amount;
        balances[_protocol][msg.sender] += _amount;

        emit Stake(_protocol, msg.sender, _amount, ranks[_protocol]);
    }
     ...
     */
    function unstake(uint256 _protocol, uint256 _amount) external {
        ranks[_protocol] -= _amount;
        balances[_protocol][msg.sender] -= _amount;
        token.safeTransfer(msg.sender, _amount);

        emit Unstake(_protocol, msg.sender, _amount, ranks[_protocol]);
    }
}
